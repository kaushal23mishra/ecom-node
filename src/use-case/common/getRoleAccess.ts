...
const getRoleAccess =
  ...
  async (userId) => {
    ...
    let routeRole = await routeRoleDb.findMany(
      { roleId: { $in: userRole ? userRole.map((u) => u.roleId) : [] } },
      {
    ...
    let roles = routeRole
      ? routeRole
          .map((rr) => rr.roleId && rr.roleId.name)
        ...
        models.forEach((model) => {
          if (routeRole && routeRole.length) {
            routeRole.map((rr) => {
              if (
                rr.routeId &&
                rr.routeId.uri.includes(`/${model.toLowerCase()}/`) &&
                rr.roleId &&
                rr.roleId.name === role
              ) {
                ...
                if (rr.routeId.uri.includes('create') && !roleAccess[role][model].includes('C')) {
                  ...
                  rr.routeId.uri.includes('list') &&
                  !roleAccess[role][model].includes('R')
                ) {
                  roleAccess[role][model].push('R');
                } else if (
                  rr.routeId.uri.includes('update') &&
                  !roleAccess[role][model].includes('U')
                ) {
                  roleAccess[role][model].push('U');
                } else if (
                  rr.routeId.uri.includes('delete') &&
                  !roleAccess[role][model].includes('D')
                ) {
