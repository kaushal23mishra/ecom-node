...
const loginUser =
  ...
  async (username, platform, password = null, roleAccess) => {
    let where: any = { $or: [{ username: username }, { email: username }] };
    where.isActive = true;
          ...
          if (limitTime > now) {
            ...
            if (!(limitTime > expireTime)) {
              return response.badRequest({
                message: `you have exceed the number of limit.you can login after ${getDifferenceOfTwoDatesInTime(now, limitTime)}.`,
              });
            }
            ...
            return response.badRequest({
              message: `you have exceed the number of limit.you can login after ${getDifferenceOfTwoDatesInTime(now, expireTime)}.`,
            });
          } else {
