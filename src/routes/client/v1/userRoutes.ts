...
router
  ...
  .post(auth(PLATFORM.CLIENT), checkRolePermission, userController.getUserCount);
router
  .route('/client/api/v1/user/:id')
  .get(auth(PLATFORM.CLIENT), checkRolePermission, userController.getUserById);
router
  .route('/client/api/v1/user/update/:id')
  .put(auth(PLATFORM.CLIENT), checkRolePermission, userController.updateUser);
router
  .route('/client/api/v1/user/partial-update/:id')
  .put(auth(PLATFORM.CLIENT), checkRolePermission, userController.partialUpdateUser);
router
  .route('/client/api/v1/user/addBulk')
  .post(auth(PLATFORM.CLIENT), checkRolePermission, userController.bulkInsertUser);
router
  .route('/client/api/v1/user/updateBulk')
  .put(auth(PLATFORM.CLIENT), checkRolePermission, userController.bulkUpdateUser);
router
  .route('/client/api/v1/user/change-password')
  .put(auth(PLATFORM.CLIENT), userController.changePassword);
router
