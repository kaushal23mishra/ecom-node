...
let filterValidationSchema = joi
  .object({
    options: options,
    ...Object.fromEntries(
      keys.map((key) => [
        ...
        joi
          .object({
            ...
            deliveryStartDate: joi
              .alternatives()
              .try(joi.array().items(), joi.date().options({ convert: true }), joi.object()),
            EstimatedDeliveryDate: joi
              .alternatives()
              .try(joi.array().items(), joi.date().options({ convert: true }), joi.object()),
            ActualDeliveryDate: joi
              .alternatives()
              .try(joi.array().items(), joi.date().options({ convert: true }), joi.object()),
            ...
            returnPickupDate: joi
              .alternatives()
              .try(joi.array().items(), joi.date().options({ convert: true }), joi.object()),
            isReturnDamaged: joi
              .alternatives()
              .try(joi.array().items(), joi.boolean(), joi.object()),
            returnRecievedDate: joi
              .alternatives()
              .try(joi.array().items(), joi.date().options({ convert: true }), joi.object()),
            shippingStatus: joi
              .alternatives()
              .try(joi.array().items(), joi.number().integer(), joi.object()),
            ...
            _id: joi
              .alternatives()
              .try(joi.array().items(), joi.string().regex(/^[0-9a-fA-F]{24}$/), joi.object()),
          })
          .unknown(true),
      ])
    ),
