      ...
      routes.forEach((route) => {
        ...
        route.methods.forEach((method) => {
          routeObj = dbRoutes.find(
            (dbRoute) => dbRoute.route_name === routeName && dbRoute.method === method
          );
          if (!routeObj) {
            routeArr.push({
    ...
    if (routeRoles && routeRoles.length) {
      ...
      let createRouteRoles = routeRoles.map((routeRole) => {
        projectRouteId = insertedProjectRoute.find(
          (pr) => pr.uri === routeRole.route.toLowerCase() && pr.method === routeRole.method
        );
...
async function seedUserRole () {
  try {
    ...
    const insertedUsers = await userDb.findMany({
    ...
    userRoles.map((userRole) => {
      ...
      if (user) {
        if (user.userType === authConstant.USER_TYPES.Admin) {
          ...
          userRolesArr.push({
            userId: user.id,
            roleId: defaultRoles.find((d) => d.code === 'CUSTOMER')._id,
          });
        } else {
          userRolesArr.push({
            userId: user.id,
