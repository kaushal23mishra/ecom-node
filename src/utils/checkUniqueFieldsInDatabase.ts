 ...
 */
const checkUniqueFieldsInDatabase = (db: any) => async (fieldsToCheck: string[], data: any, operation: string, filter: any = {}) => {
  switch (operation) {
  case 'INSERT':
  ...
  case 'REGISTER':
    ...
    if (data && data['username']){
      let loginFieldFilterForUsername: any = { $or:[] };
    ...
    if (data && data['email']){
      let loginFieldFilterForEmail: any = { $or:[] };
